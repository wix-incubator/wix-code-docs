...
+ To query all session instances within a time range of up to 1 year, use [`querySession()`](https://www.wix.com/velo/reference/wix-bookings-v2/sessions/querysessions) with the `options.startDate` and `options.endDate` parameters, and with `options.type` set to `EVENT`.
